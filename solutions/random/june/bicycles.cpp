...
void solve() {
    ...
    dist[1][s[1]] = 0;
    priority_queue<ppii, vector<ppii>, greater<ppii>> dijkstraQueue;
    dijkstraQueue.push({dist[1][s[1]], {1, s[1]}});

    while(!dijkstraQueue.empty()){
        auto [d, inner] = dijkstraQueue.top();
        auto [next, si] = inner;
        dijkstraQueue.pop();
        ...
        for (auto [v, w] : adj[next]){
            ...
            dijkstraQueue.push({dist[v][curr_s], {v, curr_s}});
        }
    }
